...
이 내장클래스도 이슈가 있어서 실제로 DB와 연동하여 사용할때 문제들을 일으킨다.
그래서 내장 Date
*/
...
router.get("/list", function (req, res) {
  // bbsVO Model을 통해서 데이터를 모두 읽어오고 (find(조건))
  ...
  bbsVO.find().then(function (bbsList) {
    ...
    // 전송을한다.
    // 현재버전인 14.x 버전에서는 { bbsList } 라고  표현하면
...
router.post("/write", function (req, res) {
  // req.body 객체에 input의 name 속성에 설정된 변수이름으로 자동으로 담겨서 전송이된다.
  ...
  // 서버에서 임의로 추가한 데이터(날짜, 시간, 카운트)를 모두 모아서 하나의 VO객체로 생성을하고
  ...
  data
    ...
    .catch(function (error) {
      ...
      // 웹화면에 데이터 추가가 잘못되었다는 메시지를 보여주는 코드를 수행해야한다.
    });
...
// PK값을 가지고 tbl_bbs에서 1개의 iteml 값을 추출하여
// detail view에 보여주기
router.get("/view/:id", function (req, res) {
  let id = req.params.id;
  // findOne : findById()
  // { where : {_id:id} }
  // tabledml _id값이 list에서 전달받은 id값과 일치하는 item이 있는지 검사
  bbsVO
    // PK(_id)깂이 id와 일치하는 데이터가 있는지 찾아라
    .findOne({ _id: id })
...
/*
데이터를 삭제할때 mongoDB 3.x 이하에서는 remove(조건문) 함수를 사용했다
조건문 없이 removoe()를 실행하면 테이블의 모든데이터를 무조건 삭제해버린다.
...
router.get("/delete/:id", function (req, res) {
  ...
  bbsVO
    /*
  mongoDB의 item(1개의 데이터를) 삭제하기 두가지 함수를 사용할 수 있다.
...
// 전달받은 PK값으로 한개의 item을 조회하고 write form에 보여주고
// 데이터를 입력한 다음 저장을 하면
...
/*
한개의 Item을 조회하여 write form으로 보낸 후 데이터를 다시 받았을 때 req.body에는 id값이 사라진 상태로 전달된다.
하지만 update를 수행할때 id값을 URL에 전달하여 보냈기 때문에 for(method="POST", action="/bbs/update/id값") 형태의 URL이
자동으로 만들어지고 POST Method는 params에 id값을 받아서 수신하는 결과가 된다.
  ...
  bbsVO
    .updateOne(
      { _id: id }, // where 조건문
      {
        ...
        // 주의해야할 코드
        // $set { req.body }
        // update를 수행하는데 실제로 form에서 전달된 데이터외의 나머지 데이터는 모두 삭제해버리는 문제가 있다.
