...
3. RestFull 방식 (Representation State Transfer)
최근에 web 어플리케이션에서 사용되는 데이터 전송방식
간단한 데이터를 서버로 보낼때 query문자열을 대체하는 방법
params 주소사용
http://localhost:3000/input/30/40/50 이런 주소를 서버로 전송하면
~/input까지는 url로 인식되고 이후 30, 40, 50 이라는 값은 params 벼수에 담아서 서버에서 사용할 수 있도록 하는 주소체계
*/

// client에서 input 주소뒤에 어떤문자열을 포함하면 nation이라는 변수에 받아서 처리할 수 있다.
...
// express를 사용한 router 설정객체를 app.js에서 import 하여 사용할 수 있도록 내보내는 기능
